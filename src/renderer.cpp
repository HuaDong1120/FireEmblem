...
void Renderer::DrawImgage(const Image&image,const Region&src,
    const Region&dst,Degrees rotation,const Vec2&center,Flags<Flip>flip)
{
    ...
    SDL_CALL(SDL_RenderTextureRotated(m_renderer,image.GetTexture(),&src_rect,&dst_rect,rotation.Value(),
        &sdl_center,static_cast<SDL_FlipMode>(static_cast<unsigned int>(flip))));
...
{
    SDL_CALL(SDL_SetRenderDrawColor(m_renderer,  color.r*255,color.g*255,color.b*255,color.a*255));
}
